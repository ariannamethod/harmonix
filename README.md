...
```
harmonix/
├── README.md              # You are here (HAiKU-specific docs)
├── requirements.txt       # Ecosystem-level (see haiku/requirements.txt for HAiKU)
├── haiku_run.py           # HAiKU launcher (bridge from root)
├── haiku/                 # HAiKU ипостась (isolated module)
│   ├── README.md          # HAiKU detailed docs
│   ├── requirements.txt   # HAiKU dependencies
...
│   ├── test_haiku_session.py  # Example generator script
│   ├── data/
...
└── sonnet/                # Sonnet ипостась (coming soon)
    └── ...
```
