"""
SonnetBrain - MLP-based sonnet quality scorer

Forked from Leo's mathbrain.py (via HAiKU) - uses micrograd-style autograd
for learning what makes a good sonnet.

Unlike formatter.validate() (rule-based), SonnetBrain LEARNS from examples.
"""
...
class SonnetFeatures:
    """Features extracted from a sonnet for MLP scoring."""
...
class SonnetBrain:
    """
    MLP-based sonnet scorer that LEARNS quality.

    Architecture: 8 inputs → 16 hidden → 8 hidden → 1 output
    Trained on (sonnet_features, human_quality_score) pairs.
    """

    def __init__(self, state_path: str = 'sonnetbrain.json'):
    ...
    def extract_features(self, sonnet: str) -> SonnetFeatures:
        """Extract features from sonnet text."""
    ...
    def score(self, sonnet: str) -> float:
        """
        Score a sonnet using MLP.
        Returns value in [0, 1] range.
        """
        features = self.extract_features(sonnet)
    ...
    def learn(self, sonnet: str, target_quality: float):
        """
    ...
    def _load_state(self):
        ...
        try:
            ...
            print(f"✓ Loaded sonnetbrain state: {self.observations} observations")

        except FileNotFoundError:
            print("⚠️  No saved sonnetbrain state, starting fresh")
...
To sleep, perchance to dream: ay, there's the rub."""
    ...
    print("Testing SonnetBrain MLP...\n")

    # Initial score
    score1 = brain.score(test_sonnet)
    print(f"Initial score: {score1:.3f}")

    # Train on this being a good sonnet
    for i in range(10):
        brain.learn(test_sonnet, target_quality=0.9)
        score = brain.score(test_sonnet)
        print(f"After training {i+1}: {score:.3f} (loss={brain.last_loss:.4f})")

    print(f"\n✓ SonnetBrain trained on {brain.observations} observations")
